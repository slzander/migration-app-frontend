...
export default function ViewMap ({ mappedBirds }) {
        ...
        const birdsArray = mappedBirds.map(bird => {
            if (bird.name === 'Upland Sandpiper'){
                return bird.locations.map(location => {
                    return <Marker position={[location.latitude, location.longitude]} icon={sandpiperIcon}></Marker>
                })
            } else {
                return bird.locations.map(location => {
                    return <Marker position={[location.latitude, location.longitude]} icon={sparrowIcon}></Marker>
                })
            }
        })

        return(
            <Map id='map' center={[0, -80]} zoom={3}>
                <TileLayer
